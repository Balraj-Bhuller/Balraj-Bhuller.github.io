...
(function () {
    ...
    function getPhaseSet() {
        var phases = new Set();
        document.querySelectorAll('.ops-entry[data-phase]').forEach(function (entry) {
            phases.add(entry.getAttribute('data-phase'));
        });
        return phases;
    }

    function waitForSection(sectionId, callback) {
        var checks = 0;
        function poll() {
    ...
    function goToArsenalPhase(status, phase) {
        var targetId = 'arsenal-' + status + '-phase-' + phase;
        if (typeof window.navigateToSection === 'function') {
            window.navigateToSection('arsenal');
            waitForSection('arsenal', function () {
    ...
    function goToFieldPhase(phase) {
        var targetId = 'field-phase-' + phase;
        if (typeof window.navigateToSection === 'function') {
            window.navigateToSection('field-operations');
            waitForSection('field-operations', function () {
                smoothScrollToTarget(targetId);
            });
            return;
        }
    ...
    function syncArsenalPhases() {
        var phaseSet = getPhaseSet();
        document.querySelectorAll('.arsenal-phase[data-phase], .phase-nav-btn[data-phase]').forEach(function (el) {
            var phase = el.getAttribute('data-phase');
            el.classList.toggle('phase-synced-hidden', !phaseSet.has(phase));
        });
    }

    function bindPhaseNavigation() {
        document.addEventListener('click', function (event) {
            var button = event.target.closest('.phase-nav-btn[data-nav-type][data-phase]');
            if (!button) return;
            event.preventDefault();
            var phase = button.getAttribute('data-phase');
            var navType = button.getAttribute('data-nav-type');
            if (navType === 'to-arsenal') {
                goToArsenalPhase(button.getAttribute('data-status'), phase);
            } else if (navType === 'to-field') {
                goToFieldPhase(phase);
            }
        });
    }
